...
//setup is that PWM23 will serve as constant high reference, inturn if the actual PWM which is altered
...
void Init_speaker_PP3 (void)
{
   ...
   PWMPER23 = 50;  //The concantenated PWM of 2 and 3, will always remain high to serve as refeence for the and gate, so when the PWM01 is ever made to be constantly low, inorder represent distance sensed is greater one meter is it will stop output to the speaker. 
   PWMDTY23 = 50;  //Duty cycle is equal to period inorder create a constant high signal
   PWMCLK   |= 0x00; //0000 0000 The reason behind this is the Clock A and B will be used and not SA and SB for PWM23 and PWM01
