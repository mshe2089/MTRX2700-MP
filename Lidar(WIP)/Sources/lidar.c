...
volatile word edge_time_1;//word?
...
void delayuptime(void){     //simple delay  using loop
  int i;
  for (i = 0; i < (int)(200 - 100*distance_copy); i++){
      delayby1ms();
  }
}


void delaydowntime(void){     //simple delay  using loop
  int i;
  ...
  for (i = 0; i < (int)(100*distance_copy); i++){
      delayby1ms();
  }
}
...
__interrupt void TOF_ISR(void) {
  overflow_count+=1;    // incrementing timer overflow counter
  TFLG2_TOF = 1;   // resetting timer interrupt flag for channel 1???????????     
}
*/